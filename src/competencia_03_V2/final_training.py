...
def preparar_datos_entrenamiento_por_grupos(
    df: pd.DataFrame,
    grupos: dict[str, list[int]],
    final_predic: int,
    undersampling_ratio: float = 0.2,
    semilla_unica: int = 555557   # solo una semilla para generar los datos
    ...
    for nombre_grupo, meses in grupos.items():
        ...
        logger.info(f"Grupo '{nombre_grupo}' con semilla {semilla_unica}: {len(X_train):,} registros")

    logger.info(f"✅ Datos preparados para {len(grupos_datos)} grupos con una sola semilla ({semilla_unica}).")
    return grupos_datos
...
def entrenar_modelos_por_grupo_y_semilla_y_parametros(
    grupos_datos: dict[str, dict[int, tuple[pd.DataFrame, pd.Series]]],
...
def entrenar_modelos_por_grupo_y_semillas(
    grupos_datos: dict[str, tuple[pd.DataFrame, pd.Series]],
    mejores_params: dict,
...
def preparar_y_append(df, nombre, 
                      ...
                      lista_ensamble):
    """
    Prepara datos de entrenamiento por grupos y los agrega a la lista de ensamble.

    Parámetros:
    - df: DataFrame de entrada
    - nombre: etiqueta para identificar el dataset
    - grupos_entrenamiento: grupos de entrenamiento (ej. FINAL_TRAINING_GROUPS_APRIL)
    - predicciones: variable de predicción (ej. FINAL_PREDIC_APRIL)
    - undersampling_ratio: ratio de undersampling
    - semilla: semilla para reproducibilidad
    - lista_ensamble: lista donde se agregan los resultados
    """
    grupos_datos = preparar_datos_entrenamiento_por_grupos(
        df,
        grupos_entrenamiento,
        predicciones,
    ...
    lista_ensamble.append((nombre, grupos_datos))
...
def entrenar_y_predecir_dataset(df, grupos, predic, mejores_params, semillas, nombre_dataset):
    ...
    for nombre_grupo, modelos_grupo in modelos.items():
        preds_grupo = np.zeros(len(X_predict), dtype=np.float32)

        for i, modelo in enumerate(modelos_grupo, start=1):
            y_pred_proba = modelo.predict(X_predict, num_iteration=modelo.best_iteration)
            preds_grupo += y_pred_proba
            ...
            ganancias.append({
                ...
                "grupo": nombre_grupo,
                "modelo_id": f"{nombre_dataset}_{nombre_grupo}_seed{i}",
                "ganancia_test": float(ganancia_test)
            })

        preds_grupo /= len(modelos_grupo)
        resultados[f"{nombre_dataset}_{nombre_grupo}"] = preds_grupo

    return resultados, ganancias, clientes_predict
